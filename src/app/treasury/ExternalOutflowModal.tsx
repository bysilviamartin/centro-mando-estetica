...
import { createExternalOutflow } from "@/actions/treasury";
import DocumentInput, { DocumentEntry } from "@/components/DocumentInput";

export default function ExternalOutflowModal({ isOpen, onClose, accounts }: { isOpen: boolean, onClose: () => void, accounts: any[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Server action
    const res = await createExternalOutflow(
  ...
  return (
    <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.6)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)" }}>
      <div className="editorial-panel" style={{ width: "100%", maxWidth: "500px", margin: "1rem" }}>
        <h2 className="brand-font" style={{ fontSize: "1.5rem", marginBottom: "1.5rem", color: "var(--danger)" }}>Nuevo Gasto / Salida</h2>
        <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", marginBottom: "2rem" }}>Registra dinero real saliendo del negocio hacia el exterior (ej: Proveedores, Impuestos, Nóminas).</p>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          ...
          <div>
            <label style={{ display: "block", fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-soft)", marginBottom: "0.5rem" }}>Entidad de Destino *</label>
            <input 
              type="text" 
              required 
              placeholder="Ej: Sumilab, Hacienda, Nombre Empleado"
          ...
          <div>
            ...
            <input 
              type="text" 
              required 
              placeholder="Motivo del pago (Factura, Nómina, etc.)"
          ...
          <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end", marginTop: "1rem" }}>
            ...
            <button type="submit" disabled={isSubmitting} className="btn-primary" style={{ backgroundColor: "var(--danger)", borderColor: "var(--danger)" }}>Guardar Salida</button>
          </div>
        </form>
      </div>
    </div>
